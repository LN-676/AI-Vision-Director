...
class FeatureRepository:
    ...
    def has_master_features(self, vehicle_id: int) -> bool:
        row = self._database.execute("SELECT COUNT(*) AS count FROM vehicle_features WHERE vehicle_id=? AND gallery_type='master'", (vehicle_id,)).fetchone()
    ...
    def summary_by_vehicle(self) -> dict[int, dict[str, int]]:
        rows = self._database.execute("SELECT vehicle_id, gallery_type, COUNT(*) AS count FROM vehicle_features GROUP BY vehicle_id, gallery_type").fetchall()
    ...
    def first_crop_jpeg(self, vehicle_id: int) -> bytes | None:
        row = self._database.execute("""SELECT crop_jpeg FROM vehicle_features WHERE vehicle_id=? AND crop_jpeg IS NOT NULL
    ...
    def prune_master(self, vehicle_id: int, limit: int) -> int:
        rows = self._database.execute("SELECT id FROM vehicle_features WHERE vehicle_id=? AND gallery_type='master' ORDER BY quality_score, created_at", (vehicle_id,)).fetchall()
        ids = [int(row["id"]) for row in rows[:max(0, len(rows) - limit)]]
        if not ids:
            return 0
        cursor = self._database.execute(
            f"DELETE FROM vehicle_features WHERE id IN ({','.join('?' for _ in ids)})",
            ids,
            commit=True,
        )
        return int(cursor.rowcount or 0)
    ...
    def _ensure_schema(self) -> None:
        self._database.execute("""CREATE TABLE IF NOT EXISTS vehicle_features (
            ...
            embedding_json TEXT NOT NULL, crop_jpeg BLOB, metadata_json TEXT)""", commit=True)
