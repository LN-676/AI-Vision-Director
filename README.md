...
| 元件 | 正式名稱 | 主要責任 |
| --- | --- | --- |
| Desktop | AI Vision Director Desktop V2.0 | PySide6／Tkinter 介面、AI 偵測、追蹤、ReID、構圖、WebSocket Server、資料庫與評估 |
| iOS | AI Vision Director Camera for iOS V2.0 | 相機擷取、JPEG 串流、Bonjour 探索、WebSocket Client、DockKit 控制與安全停止 |

目前正式產品版本是 **V2.0**。PySide6 方案 A 是主要模組化工作區；既有 Tkinter 入口仍保留。產品版本升級不改變 1.0 WebSocket contract、SQLite 格式、cache 路徑、Bonjour service type 或 DockKit safety policy。舊的 V1.77 程式碼保留在 Git tag `v1.77`。
...
flowchart LR
    subgraph COMPUTER["Mac／電腦端"]
        DESKTOP["AI Vision Director Desktop V2.0<br/>PySide6 Scheme A／Tkinter compatibility"]
    ...
    subgraph MOBILE["iPhone＋手機穩定器"]
        IOS["AI Vision Director Camera for iOS V2.0"]
...
flowchart TD
    ...
    QTPANELS["Qt Panels<br/>Source pages · Vehicle DB · Feature Manager"]
...
- `ui_qt/`：PySide6 QMainWindow、Before／After 雙監看、模組化 Dock、Workspace、來源分頁、Vehicle Database 與 Feature Manager。
...
```text
AI-Vision-Director/
├── src/autocamtracker/       # Desktop V2.0 Python application and shared use cases
│   ├── ui_qt/                # PySide6 Scheme A workspace
│   └── ui/                   # Preserved Tkinter compatibility UI
├── tests/                    # Desktop unit and integration tests
├── ios/DockKitTester/        # iOS V2.0 Xcode project and Swift tests
...
`DockKitTester` 是目前保留的 Xcode 內部 target／資料夾名稱；App 顯示名稱與產品文件均為 **AI Vision Director Camera for iOS V2.0**。
...
- 最新程式碼：`main`
- 目前發布：`AI-Vision-Director-V2.0`
- 舊版封存：`v1.77` 與其他歷史 tags
- Desktop 與 iOS 使用同一個 V2.0 產品版本，並維持 1.0 WebSocket contract 相容。
...
| --- | --- | --- |
| Desktop | AI Vision Director Desktop V2.0 | PySide6/Tkinter delivery layers, detection, tracking, ReID, framing, WebSocket server, persistence, and evaluation |
| iOS | AI Vision Director Camera for iOS V2.0 | Camera capture, JPEG streaming, Bonjour discovery, WebSocket client, DockKit control, and safety stop |

The current product release is **V2.0**. PySide6 Scheme A is the primary modular workspace and the Tkinter entry point remains available. The product release does not change the 1.0 WebSocket contract, SQLite format, cache paths, Bonjour service type, or DockKit safety policy. The previous V1.77 source remains available through the `v1.77` Git tag.
...
- `ui_qt/`: PySide6 QMainWindow, Before/After monitors, modular docks, workspaces, source pages, Vehicle Database, and Feature Manager.
...
- Latest code: `main`
- Current release: `AI-Vision-Director-V2.0`
- Archived releases: `v1.77` and earlier tags
- Desktop and iOS share the V2.0 product version while retaining the compatible 1.0 WebSocket contract.
