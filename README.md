# AutoCamTracker V1.71

AutoCamTracker 是一個以影片、螢幕區域、webcam 或 iPhone 作為輸入的車輛偵測、單車追蹤、數位構圖與 GID 重新辨識工具。V1.71 加入 iPhone 實體相機變焦控制、15 FPS iPhone 串流、自動連線與啟動、ReID 快取與預測走廊、DockKit 安全控制，以及 Fixed Cut／AI Tracking／In-Out Auto 賽道拍攝模式。
...
- `src/autocamtracker/main.py`：目前 V1.71 桌面版 Tkinter app 啟動入口。
...
- `src/autocamtracker/server/websocket_server.py`：V1.71 桌面端與 DockKit iOS app 的 WebSocket bridge。
...
2. V1.71 預設選擇 `iphone`，會在 `8765` port 啟動 WebSocket Server 並自動啟動影像管線；畫面會顯示可連線網址與 Copy 按鈕。
3. iOS App 會使用已保存的 URL 自動連線；需要更換 Mac 時，也可在連線頁輸入 `ws://<Mac 位址>:8765/ws/tracking` 後按 `Connect`。
4. iOS App 會以最高約 15 FPS 傳送 JPEG 相機畫面給 V1.71；桌面端完成 YOLO、BoT-SORT 與 GID ReID 後，再透過相同 WebSocket 回傳 tracking command 與實體相機 `zoom_factor`。
...
- V1.71 仍以單一車輛追蹤與互動式 GID 管理為主。
- V1.71 新增 iPhone frame timestamp envelope、Mac 端 decode / receive / pipeline telemetry、iPhone preview 節流與控制指令優先送出，降低 DockKit 控制路徑延遲。
- V1.71 的 GID reacquire 可暫時鎖定 `LID=None` 的 detection，並使用 NumPy 加速 Master feature gallery 比對。
- 實體變焦已接到 iOS 相機端；若 telemetry 的 `camera_display_zoom_factor` 仍不動，優先檢查 iOS App 是否為 1.71、是否收到 `zoom_factor`、以及相機硬體可用倍率範圍。
- `Auto Add Feature` 會寫入 Master gallery；如果 Master 已存在，新增 feature 會先通過 class 與 ReID 防污染檢查。
- 如果 YOLO / tracker 產生 `LID=None`，V1.71 會先用 visual temporary lock 保持 GID；等 tracker 後續恢復 LID 時再重新綁定。
- 若 GID 已被舊版本寫入錯車 feature，V1.71 會防止繼續污染，但不會自動清理既有髒資料。
- Identity DB 是本機資料，預設位於 `outputs/vehicle_identity.sqlite3`，不應作為 release 檔案上傳。
- `outputs/`、`.venv/`、cache、測試輸出都不屬於乾淨 release 內容。

---

# AutoCamTracker V1.71 English

AutoCamTracker is a vehicle detection, single-target tracking, digital reframing, and GID re-identification tool. V1.71 adds physical iPhone camera zoom control, a 15 FPS iPhone stream, automatic connection and startup, cached and trajectory-gated ReID, safer DockKit control, and racetrack Fixed Cut, AI Tracking, and In/Out Auto shot modes.
