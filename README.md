...
- 一般點選 bbox 只會選取目前 LID；按 `Add Vehicle` 後再點 bbox，才會建立新的 GID 資料庫欄位。
- 選取 Identity DB 的 GID row 後按 `Link BBox`，下一次點 bbox 才會綁定到該 GID。
...
5. 按 `Start` 開始偵測。
6. 按 `Add Vehicle`，再到 Before 畫面點選車輛 bbox 建立 GID。
7. 需要補綁既有 GID 時，先點 Identity DB 的 GID row、按 `Link BBox`，再點 Before 畫面中的 bbox。
