...
目前桌面介面預覽版本是 **V2.0 beta1**；既有 V1.0 Tkinter 入口與 1.0 WebSocket contract 保持相容。舊的 V1.77 程式碼保留在 Git tag `v1.77`，不再出現在最新 `main` 的工作目錄。
...
V2.0 beta1 的 PySide6 方案 A 預覽入口：
...
The desktop UI preview is **V2.0 beta1**. The V1.0 Tkinter entry point and the 1.0 WebSocket contract remain compatible. The previous V1.77 source remains available through the `v1.77` Git tag and is no longer the working tree shown on `main`.
