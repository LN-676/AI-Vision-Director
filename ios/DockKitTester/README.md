...
- V1.3 `TrackingCommand` JSON parser、dead zone、速度上限、smoothing。
- 真實 WebSocket Client、V1.41 URL 輸入、自動重連、假 JSON 注入與 500 ms timeout STOP。
...
11. 按 `Inject Fake JSON`，確認 V1.3 資料可解碼、控制迴路有輸出，500 ms 後自動停止。
12. 測試完成後按 `Exit / Tracking ON` 恢復系統追蹤。
13. 複製 API Log 保存結果。判斷相容性的關鍵是 `Manual Mode ready` 與各能力測試是否成功。

## 連接 AutoCamTracker V1.41

1. Mac 的 V1.41 選擇 `iphone`，等待畫面顯示 WebSocket URL。
2. iPhone 與 Mac 使用同一 Wi-Fi，或先以 USB-C 建立 Personal Hotspot USB / USB Ethernet IP 網路。
3. App 的 `AutoCamTracker V1.41` 區輸入 Mac 顯示的完整 URL，例如 `ws://MacBook.local:8765/ws/tracking`，再按 `Connect`。
4. V1.41 顯示 `iPhone connected` 後，先讓 DockKit 進入 Manual Mode，再執行桌面端測試脈衝。
