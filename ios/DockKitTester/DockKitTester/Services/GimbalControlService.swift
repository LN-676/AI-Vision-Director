...
final class GimbalControlService: ObservableObject {
    ...
    private let calibrationKey = "AutoCamTrackerGimbalCalibrationV177"
    ...
    func apply(_ trackingCommand: TrackingCommand) async {
        guard trackingCommand.type == "tracking" else {
            logger.log(.error, "Ignored V1.77 message with unsupported type: \(trackingCommand.type).")
            await emergencyStop(reason: "invalid V1.77 message")
            return
        }
