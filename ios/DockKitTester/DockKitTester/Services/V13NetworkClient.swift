...
final class V13NetworkClient: ObservableObject {
    ...
    private func receiveTrackingCommand(_ data: Data) async {
        ...
        case .success(let command):
            guard sequenceValidator.accept(command) else {
                logger.log(.error, "V1.71 JSON rejected: duplicate or out-of-order sequence.")
            ...
            logger.log(
                .success,
                String(format: "V1.71 JSON decoded: locked=%@ error=(%.3f, %.3f) confidence=%.2f zoom=%@ predicted=%@.", String(command.targetLocked), command.errorX, command.errorY, command.confidence, command.zoomFactor.map { String(format: "%.2f", $0) } ?? "nil", String(command.predictedTarget ?? false))
            )
        ...
        case .failure(let error):
            logger.log(.error, "V1.71 JSON decode failed: \(error.localizedDescription)")
    ...
    func sendFakeCommand() async {
        let json = #"{"type":"tracking","version":"1.0","source_version":"1.71","target_locked":true,"target_id":7,"error_x":0.18,"error_y":-0.04,"confidence":0.91,"timestamp_ms":1781770000000,"zoom_factor":2.0}"#
        logger.log(.info, "Injecting a fake V1.71 JSON command.")
    ...
    private func armTimeout() {
        ...
        timeoutTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                await triggerTimeout(reason: "no V1.71 data for 500 ms")
            } catch {
                return
            }
        }
    ...
    private func triggerTimeout(reason: String) async {
        timeoutTask?.cancel()
        status = .timedOut
        logger.log(.warning, "V1.71 safety timeout: \(reason).")
        await onTimeout?()
    }
}
