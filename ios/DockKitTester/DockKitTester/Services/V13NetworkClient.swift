...
final class V13NetworkClient: ObservableObject {
    ...
    func receive(data: Data) async {
        ...
        case .success(let command):
            guard command.type == "tracking" else {
                logger.log(.error, "V1.43 JSON rejected: type must be 'tracking'.")
            ...
            logger.log(
                .success,
                String(format: "V1.43 JSON decoded: locked=%@ error=(%.3f, %.3f) confidence=%.2f.", String(command.targetLocked), command.errorX, command.errorY, command.confidence)
            )
            await onCommand?(command)
            armTimeout()
        case .failure(let error):
            logger.log(.error, "V1.43 JSON decode failed: \(error.localizedDescription)")
    ...
    func sendFakeCommand() async {
        let json = #"{"type":"tracking","version":"1.0","source_version":"1.43","target_locked":true,"target_id":7,"error_x":0.18,"error_y":-0.04,"confidence":0.91,"timestamp_ms":1781770000000}"#
        logger.log(.info, "Injecting a fake V1.43 JSON command.")
    ...
    private func armTimeout() {
        ...
        timeoutTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                await triggerTimeout(reason: "no V1.43 data for 500 ms")
            } catch {
                return
            }
        }
    ...
    private func triggerTimeout(reason: String) async {
        timeoutTask?.cancel()
        status = .timedOut
        logger.log(.warning, "V1.43 safety timeout: \(reason).")
        await onTimeout?()
    }
}
